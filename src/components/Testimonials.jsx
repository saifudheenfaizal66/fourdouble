...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-16 sm:py-24 relative overflow-hidden bg-[#F8FAFC]">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Testimonials Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 max-w-7xl mx-auto gap-5 sm:gap-8">
          {reviews.map((rev, idx) => (
            <div
              key={idx}
              className="p-5 sm:p-8 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between hover:-translate-y-1"
            >
              <div>
                ...
                <p className="text-xs sm:text-sm lg:text-base leading-relaxed mb-5 sm:mb-6 italic text-slate-700 font-normal">
                  "{rev.content}"
                </p>
              </div>

              <div className="pt-3.5 sm:pt-4 border-t flex flex-col xs:flex-row xs:items-center justify-between border-slate-100 gap-2">
                <div>
                ...
                <span className="text-[10px] sm:text-xs font-extrabold uppercase px-2.5 sm:px-3 py-1 rounded-full border bg-blue-50 text-[#0077B6] border-blue-200 self-start xs:self-auto shrink-0">
                  {rev.service}
                </span>
              </div>
